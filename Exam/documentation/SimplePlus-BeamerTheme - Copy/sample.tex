  ...
  \begin{columns}
    \begin{column}{0.5\textwidth}
      \begin{block}{Generation Workflow}
        \begin{enumerate}
          \item \textbf{Prompt Engineering}: Structured prompts for non-toxic conversations
          \item \textbf{LLM Generation}: Google Gemini API generates Italian dialogues
          \item \textbf{Response Parsing}: Extract conversation components (names, dialogue, type)
        \end{enumerate}
      \end{block}
      
      \begin{block}{Safety Measures}
        \begin{itemize}
          \item Safety filters to block harmful content
          \item Retry mechanism for failed generations
          \item Manual validation of conversation quality
        \end{itemize}
      \end{block}
    \end{column}
